...
class PodcasterStatsMysql extends PodcasterStats implements PodcasterStatsInterface
{
    ...
    public function upsertFeed($feed): void
    {
        ...
        $query = 'INSERT INTO feeds(' . implode(',', $fields) . ') VALUES("' . implode('","', $values) . '") ON DUPLICATE KEY UPDATE downloads = downloads+1;';
    ...
    public function upsertEpisode($feed, $episode, $trackingDate): void
    {
        ...
        $query = 'INSERT INTO episodes(' . implode(',', $fields) . ') VALUES("' . implode('","', $values) . '") ON DUPLICATE KEY UPDATE downloads=downloads+1;';
        $this->database->execute($query);
    }
    ...
    public function upsertUserAgents($feed, array $userAgentData, int $trackingDate): void
    {
        ...
        $query = 'INSERT INTO os(' . implode(',', $fields) . ') VALUES("' . implode('","', $values) . '") ON DUPLICATE KEY UPDATE downloads=downloads+1;';
        $this->database->execute($query);
        ...
        $query = 'INSERT INTO useragents(' . implode(',', $fields) . ') VALUES("' . implode('","', $values) . '") ON DUPLICATE KEY UPDATE downloads=downloads+1;';
        $this->database->execute($query);
        ...
        $query = 'INSERT INTO devices(' . implode(',', $fields) . ') VALUES("' . implode('","',    $values) . '") ON DUPLICATE KEY UPDATE downloads=downloads+1;';
        $this->database->execute($query);
    }
